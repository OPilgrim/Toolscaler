...
def main(
    ...
    if corpus == "G123":
        documents_df = pd.read_csv('data/retrieval/corpus_G123.tsv', sep='\t')    # 所有 docid-API 的对应信息，49936条
        print("Using G123 corpus")
    else:
        documents_df = pd.read_csv(os.path.join(data_path, 'corpus.tsv'), sep='\t')  # 各个子集包含的docid-API信息，G1 432800条，G2 570931条，G3 202846条，但是里面重复的条目太多了，也不知道当初为什么要弄成这样，好在下面有做去重处理
    ...
    labels_df = pd.read_csv(os.path.join(data_path, rel_path), sep='\t', names=['qid', 'useless', 'docid', 'label'])  # qid是query，对应多个docid，每个docid对应一个API
    for row in labels_df.itertuples():
        ir_relevant_docs.setdefault(row.qid, set()).add(row.docid)  # 这个也是需要去重的
    ...
    if corpus == "G123":
        with open(f"data/retrieval/{stage}_toolid_to_full_tool_id.json", "r") as f:
            domain_toolid_to_full_toolid = json.load(f)   # domain？怎么就domain了
        new_ir_relevant_docs = {}
        for qid, docids in ir_relevant_docs.items():
            new_ir_relevant_docs[qid] = set()
            for docid in docids:
                try:
                    new_ir_relevant_docs[qid].add(domain_toolid_to_full_toolid[str(docid)])   # 这句应该是为了兼容哪个实验，所以G123到底对应哪个实验？？
                except KeyError:
    ...
    indexer = BM25Indexer(corpus, ids=corpus_ids)   # 建立索引
    ...
    def compute_ndcg(relevant_docs_ids, score_docid, k):
        ...
        for hit in top_hits:
            ...
            if hit[1] in relevant_docs_ids:    # 这是一个query的relevant_docs，这个ndcg算的是单个query的
    ...
    scores_docids = indexer.search(queries, top_n=100)   # 每个query返回100个item，这里应该是两层的吧
    result_scores = {}
    for k in [1, 3, 5]:
        ndcg_scores = []
        ...
        result_scores[f"ndcg@{k}"] = np.mean(ndcg_scores)   # 确实，ndcg只能一个一个算，最后再取平均
