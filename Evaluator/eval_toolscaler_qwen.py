# To evaluate NDCG, the code is sourced from /data/suyunyue/Projects/tool/ToolGen/evaluation/retrieval/eval_toolgen.py
...
def constrained_beam_search(query, model, tokenizer, device, trie, constrain=True, template="qwen-7b-chat"):
...
def main(
    model_name_or_path="models/qwen2.5-full/sft/gist_ret_2@2_15",
    indexing="qwen15",
    stage="G2",  # G1, G2, G3
    split="category",  # instruction, tool, category
    result_path="Results/Rec@NDCG",
    constrain="True",
    limit_to_stage_space="False",
    template="qwen-7b-chat",
    corpus="G123",  # G1, G2, G3, G123
):
    ...
    if corpus == "G123":
        documents_df = pd.read_csv('data/retrieval/corpus_G123.tsv', sep='\t')    # All the corresponding information for the docid-API, a total of 49,936 items.
        print("Using G123 corpus")
    else:
        documents_df = pd.read_csv(os.path.join(data_path, f'corpus.tsv'), sep='\t')
        print(f"Using {data_path} corpus")
    ir_corpus = process_retrieval_ducoment(documents_df)   # {docid: toolbench_name}
